...
client.on('guildMemberAdd', async member => {
  ...
  connection.query(`SELECT welcome_msg, welcome_msg_content, welcome_channel_id, welcome_role, welcome_role_id FROM Guild WHERE guild_id = ${member.guild.id}`, async (err, results) => {
    await resolve(1)
    if(err) console.error(err)
    else {
      ...
      if(results[0].welcome_msg === 0);
        ...
        if(results[0].welcome_msg_content == null)
            ...
            welcome_channel.send(`Hey, ${member}. ` + results[0].welcome_msg_content)
      }
...
client.on('message', async message => {
  ...
  if(message.content.startsWith(prefix + 'lock')){
    ...
    if(message.author.id != 366345779687981056 && message.author.id != 359648176791355403) return message.reply('you do not have the permission to do that.')
